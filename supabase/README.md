...
This directory is a **scaffold**: SQL migrations, RLS policies, and two
edge functions for a real backend, built entirely as code so they can be
reviewed and run later against your own Supabase project. There is no
live Supabase project or credentials behind this — nothing here has
been deployed, and **nothing in the app calls this backend yet**. The
mobile app still reads everything from `mobile/src/data/mock.ts`. This
is groundwork for a future integration, not a live one.
...
5. **Deploy the two edge functions**:
   ...
   ```
   Both read `SUPABASE_URL`, `SUPABASE_ANON_KEY`, and
...
- **Nothing calls this backend yet.** `mobile/src/lib/supabase.ts`
  exports a typed client factory (`getSupabaseClient()`) but no screen
  imports it. Wiring real screens up to real queries — including
  calling `generate-family-code` / `redeem-family-code` from
  `mobile/src/lib/deviceBinding.ts` — is a follow-up.
- **No child auth.** There is no separate Supabase auth role for a
  ...
  `supabase/migrations/0004_rls_policies.sql`. A real device-scoped
  child session (so a child's device can talk to the database
  directly, not just through the two edge functions) is future work.
